...
#else
//code taken from https://gavv.github.io/articles/file-locks/
	struct flock fl;
...
//initialize the struct
	...
	/** move to the right location */
		if (lseek(lockFileHandle, offset, SEEK_SET) < 0) {
				fprintf(stderr, "PID %d: failed seeking to position %ld : %d %s\n",
		...
		if (operation == 'X') {
			...
			if (fcntl(lockFileHandle, F_SETLKW, &fl) < 0) {
					fprintf(stderr, "PID %d: failed     on %s lock of %ld bytes at %ld : %d %s\n",
									getpid(), operationTag,
									length, offset, errno, strerror(errno));
					return -1;
			} else {
					printf("PID %ld: granted    %s at %ld for %ld bytes at %s",
			...
			fl.l_type = F_UNLCK;
			if (fcntl(lockFileHandle,  F_SETLK, &fl) < 0) {
					fprintf(stderr, "PID %d: failed     on %s lock of %ld bytes at %ld : %d %s\n",
									getpid(), operationTag,
									length, offset, errno, strerror(errno));
					return -1;
			} else {
					printf("PID %ld: unlocked   %s at %ld for %ld bytes at %s",
									(long) getpid(), operationTag, offset, length,
									curtimeString());
			}
		}
			...
			fl.l_type = F_WRLCK;
			if (fcntl(lockFileHandle,  F_GETLK, &fl) < 0) {
					fprintf(stderr, "PID %d: failed     on %s lock of %ld bytes at %ld : %d %s\n",
									getpid(), operationTag,
									length, offset, errno, strerror(errno));
					return -1;
				...
				fprintf(stderr, "Shared lock: PID: %d length: %ld Offset: %ld Calculated from: %hi\n", getpid(), (long)fl.l_len, (long)fl.l_start, fl.l_whence);
			}
			else if (fl.l_type == F_WRLCK){
				fprintf(stderr, "Exclusive lock: PID: %d length: %ld Offset: %ld Calculated from: %hi\n", getpid(), (long)fl.l_len, (long)fl.l_start, fl.l_whence);
			}
			else{
